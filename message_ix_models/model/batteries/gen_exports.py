...
def gen_exports(file, sheet_main, scenario):
    # mp = ixmp.Platform()
    ...
    for t in technologies:
        ...
        if (
            "technical_lifetime"
            in data.loc[(data["technology"] == t)]["parameter"].values
        ):
            lifetime = data.loc[
                (data["technology"] == t) & (data["parameter"] == "technical_lifetime"),
                "value",
            ].values[0]
        ...
        for vtg in years_df["year_vtg"].unique():
            ...
            years_df_final = pd.concat(
                [years_df_temp, years_df_final], ignore_index=True
            )
        vintage_years, act_years = (
            years_df_final["year_vtg"],
            years_df_final["year_act"],
        )

        params = data.loc[(data["technology"] == t), "parameter"].values.tolist()
        ...
        for par in params:
            ...
            val = data.loc[
                ((data["technology"] == t) & (data["parameter"] == par)), "value"
            ].values[0]
